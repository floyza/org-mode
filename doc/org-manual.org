...
=$name= is interpreted as the name of a column, field, parameter or
constant.
...
kilometers[fn:8].  Column names, field names and parameters can be
specified in special table lines.  These are described below, see
[[*Advanced features]].  All names must start with a letter, and further
...
like =$4=$1+$2=.  You can also use named columns, like =$NAME=$1+$2=
...
- =!= ::
  ...
  may refer to a column as =$Tot= instead of =$6=. These names have
  precedence over field names if a column and field share the same
  name.
